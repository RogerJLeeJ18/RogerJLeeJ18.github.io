...
_.first = function(arr, number) {
    if (Array.isArray(arr) === false || number < 0) {
        return [];
    } else if (!number) {
        return arr[0];
    } else {
...
_.partition = function(array, func) {
    // var arr = [];
    // var arr2 = [];
    // arr 1 = filter(array, func);
    //  arr 2 = reject(array, func);
    // arr.push(arr1);
    // arr.push(arr2);
    // return arr;
...
_.map = function(collection, func) {
    ...
    _.each(collection, function(element, index, collection) {
        ...
        _.each(collection, function(value, key, collection) {
            arr.push(func(value, key, collection));
        });
    }
    return arr;
};
...
_.pluck = function(objarray, property) {
    var arr = [];
    _.map(objarray, function (element, index, array) {
...
_.every = function(collection, func) {
    ...
    if (_.contains(arr, false)) {
            return false;
    } else if (!_.contains(arr, false)) {
            return true;
    }
};    
    
    // var arr = _.map(collection, function(element, index, collection) {
    //          func(element, index, collection)

    // })
    // console.log(arr)
// // if (arr contains all trues return true)
//     if (_.each(collection, function(element, index, collection) {
//             func(element, index, collection)  
//         }) === false) {
//         return false;
//     } else if (_.each(collection, function(element, index, collection) {
//             func(element, index, collection);  
//         }) === true) {
//         return true;
//     }
...
_.extend = function (obj1, obj2) {
    /* i wanna loop thru my other objects and add their keys & values to the first obj if it already has
    that key jus replace the value
    */
    for (var i = 1; i < arguments.length; i++) {
        obj2 = arguments[i]; 
        _.each(obj2, function(value, key, obj2) {
            obj1[key] = value;
        });
    }
    return obj1;
