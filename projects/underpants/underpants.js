...
_.unique = (array) => {
    var uniqArr = [];
    _.each(array, function(element, index, array) {
        if(_.indexOf(uniqArr, element) === -1) {
...
_.map = (collection, action) => {
    ...
    if (Array.isArray(collection)) {
        _.each(collection, function(element, index, collection) {
            array.push(action(element, index, collection));
        });
    } else if (typeof collection === 'object') {
        _.each(collection, function(value, key, collection) {
           array.push(action(value, key, collection)); 
        });
    }
    return array;
};
...
//   _.each(array ,(e, i, a) => {
//      // if no seed is given seed becomes the first element of the collection
//      if(!seed) {
//       seed = array[0];
//      } else {
//       // otherwise, perform the action on seed, element & index
//       // set the result of that function call to the seed
...
_.extend = (...args) => {
   debugger; 
   return _.reduce(args, function(previous, current, index) {
